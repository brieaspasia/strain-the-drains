...
#create survey count  
        ...
        #load item counts
        item <- read.csv(file="Data/200630_micro.csv", 
                         header=T, sep=",") 
        ...
        survey_count <- survey_count %>%
          ...
          mutate(event_item = paste(event_id, Type, sep=" "))
...
#create abundance data with all possible debris items and zero values
      ...
      type <- as.data.frame(unique(event_count$Type))
      
      
      #create a df of all possible items at all survey events, complete with zeros
      abund <- full_join(event_count, type, by=c("Type" = "unique(event_count$Type)"))
...
#join abundance data to survey notes and other columns
      df <- left_join(abund_long, survey_count, by=c("event_id" = "event_id", "variable" = "Type"))
      
      df <- df %>%
        dplyr::select(-event_item, -event_date, -sum) %>% #remove duplicate columns
        rename(item=variable, sum=value) %>% #rename columns
