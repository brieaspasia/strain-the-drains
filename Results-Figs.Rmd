...
# Purrr
pred_main_df = purrr::map_df(d_main_vec, function(x){
...
main_df <- pred_main_df %>% 
  filter(!item %in% c("Beer bottles (and similar)", "Foam take-away", "Plastic drink bottles")) %>%
  mutate(LUZ = as.factor(LUZ),
  item=as.factor(item),
  order=reorder_within(item, Total, LUZ))

p_pred_main_df <- 
  ggplot(main_df, aes(x=covid, y=Total, fill=LUZ)) +
  geom_bar(position = "dodge", stat="identity") +
