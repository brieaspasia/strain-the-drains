...
item_per_cycle <- df %>%
  ...
  mutate(LGAcycle=ifelse(date=="2020-10-12", 2, 6),
         LGAsum=cycle_sum/LGAcycle)
...
p <- ggplot() +
  ...
  geom_line(data=item_per_cycle, aes(date, LGAsum, linetype=LUZ, color=LUZ)) +
...
#Need to loop every LGA separately with the land uses for supplementary, then choose one to talk about in results.  For Hollywood Squares, use LGA as line, not land use.
```
```{r avg-items-per-lockdown-LGA, warning=FALSE, message=FALSE}
#find top 15 items
top15 <- df %>%
  filter(item != "Metal scrap") %>% #removing outlier entry
  group_by(item) %>%
...
p <- ggplot() +
  geom_rect(data=covid_lvl, aes(NULL, NULL, xmin=start, xmax=end, fill=level, 
                                ymin= -Inf, ymax=Inf), alpha=0.1) +
  labs(fill="Covid-19 lockdown", linetype="LGAs", colour="LGAs",
       x="Date", y="Debris count per survey cycle") +
  # geom_point(data=item_per_cycle, aes(date, cycle_sum, color=LUZ)) +
  geom_line(data=item_per_cycle, aes(date, cycle_sum, linetype=LGA, color=LGA)) +
  theme(axis.text.x=element_text(angle=45, vjust=0.75, hjust=0.8)) +
  facet_wrap(~item, scales="free_y") +
  geom_vline(xintercept = as.numeric(as.Date("2020-06-30")), linetype=4)
p
...
```{r avg-items-per-lockdown-endcycle7, warning=FALSE, message=FALSE}
#find top 15 items
top15 <- df %>%
  filter(item != "Metal scrap") %>% #removing outlier entry
  group_by(item) %>%
...
item_per_cycle <- df %>%
  filter(item %in% top15 | item %in% pol_items) %>%
  filter(date != "2020-10-12") %>% #removing last survey cycle
  group_by(date, covid, LUZ, item) %>%
  summarise(cycle_sum = sum(sum)) 
...
covid_lvl <- data.frame(name = c("1", "2"),
                   start = as.Date(c("2019-10-29", "2020-03-15")),
                   end = as.Date(c("2020-03-15", "2020-07-12")),
                   level = c("Pre-Covid", "During Covid"),
                   stringsAsFactors = FALSE) %>%
  mutate(median_x = start + floor((end-start)/2))
...
p <- ggplot() +
  geom_rect(data=covid_lvl, aes(NULL, NULL, xmin=start, xmax=end, fill=level, 
                                ymin= -Inf, ymax=Inf), alpha=0.1) +
  labs(fill="Covid-19 lockdown", linetype="Land use", colour="Land use",
       x="Date", y="Debris count per survey cycle") +
  # geom_point(data=item_per_cycle, aes(date, cycle_sum, color=LUZ)) +
