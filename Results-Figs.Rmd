...
micro_pct_change <- micro_df %>%
  group_by(covid, LUZ, asset_id) %>% #look at items per asset both cov
  mutate(assetMean=mean(sum)) %>% #mean of item btwn 8 cycles at each LUZ per covid level
  ungroup() %>%
  dplyr::select(covid, LUZ, asset_id, assetMean) %>% #select vars
  unique() #remove duplicates from survey cycle
