...
pct_change <- df %>%
  filter(item %in% top15 | item %in% pol_items) %>%
  group_by(covid,item, LUZ) %>%
  summarise(Total=sum(sum)) %>%
  spread(covid, Total) %>%
  rename(precovid=3, duringcovid=4) %>%
  mutate(diff=precovid-duringcovid,
         pct_decrease=diff/precovid*100)

change <- pct_change %>%
  dplyr::select(item, LUZ, pct_decrease) %>%
  unique() %>%
  spread(item, pct_decrease) %>%
  mutate(LUZ = as.factor(fct_relevel(LUZ, "CBD", "Shopping Centre", "Public Transport Terminal", "Industrial Precinct"))) %>%
  arrange(LUZ) %>%
  column_to_rownames(var="LUZ")

mat <- data.matrix(change) #matrix for heatmap
# mat <- reorder_mat(mat = mat, order = order)
mat <- round(mat,0)
# mat2 = ifelse(mat <= 0.99,"<1",as.character(round(mat,2))) #matrix for labels
...
heatmap_change <- Heatmap(log10(mat), 
        name = "Debris count (percentage change per land use zone)", 
        heatmap_legend_param  = list(color_bar = "continuous", at = c(-2.6,3.5),
                      title = "Debris counts (% total)",labels = c("Low", "High"),
                      grid_width = unit(0.8, "cm"),
                      legend_height = unit(5, "cm"),
        ...
        #percentage label
        cell_fun = function(j, i, x, y, width, height, fill) {
        grid.text(sprintf("%s", mat[i, j],"%"), x, y, gp = gpar(fontsize = 10))},
        )
