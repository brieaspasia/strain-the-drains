...
```{r emmeans-interactions-foam}
mod_foam <- df %>%
  filter(item=="Foam insulation & packaging") %>%
  group_by(event_id, asset_id, cycle, date, covid, LGA, LUZ) %>% #relevant variables
  summarise(sum=sum(sum)) %>%
  ungroup() %>%
  mutate(LGA = as.factor(LGA),
...
#create model
m_foam  <- glmmTMB(sum ~ #Debris count per survey event
                LUZ * covid + #predicted by land use effected by covid
               (1|LGA/asset_id) + (1|cycle), #suburb random effect
               family=nbinom2(), #negative binomial to deal with count data and zeros
               data = mod_foam)

emmeans(m_foam, pairwise~covid|LUZ, type="response")

joint_tests(m_foam)

emmip(m_foam, ~covid | LUZ)

foam.rg <- ref_grid(m_foam)
foam.rg
plot(foam.rg, by = "LUZ")

emmeans(m_foam, type="response")
...
#create model
m_cigs  <- glmmTMB(sum ~ #Debris count per survey event
                LUZ * covid + #predicted by land use effected by covid
               (1|LGA/asset_id) + (1|cycle), #suburb random effect
               family=nbinom2(), #negative binomial to deal with count data and zeros
               data = mod_cigs)
...
emmip(m_cigs, ~covid | LUZ)
plot(cigs.rg, by = "covid")

ci <- confint(cigs.rg, level = 0.95)
...
predict(cigs.rg, interval = "prediction")
plot(cigs.rg, PIs = TRUE)
...
delta_LUZ = purrr::map_df(ls.data_split, function(x){
...
```{r percent-change-delta-macro}
#refer to delta method
#https://stats.stackexchange.com/questions/64652/calculating-standard-deviation-associated-with-percentage-change
pct_change <- df %>%
  group_by(covid, asset_id, LGA) %>% #look at items per asset both cov
  mutate(assetMean=mean(sum)) %>% #mean of item btwn 8 cycles at each asset per covid level
  ungroup() %>%
  dplyr::select(covid, asset_id, assetMean) %>% #select vars
  unique() #remove duplicates from survey cycle

# delta = purrr::map_df(pct_change, function(x){

x1 = pct_change %>% filter(covid == 1)
# x1$assetMean = ifelse(x1$assetMean == 0, 0.00001, x1$assetMean)

x2 = pct_change %>% filter(covid == 2) %>% dplyr::select(asset_id, assetMean)
# x2$assetMean = ifelse(x2$assetMean == 0, 0.00001, x2$assetMean)
