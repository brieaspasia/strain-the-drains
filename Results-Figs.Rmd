...
# pred_luz_items <- pred_luz_items %>%
#   mutate(item=reorder_within(item, Total, LUZ))


p_pred_luz_items <- 
  ggplot(pred_luz_items, aes(x=Total, y=reorder(LUZ, Total), fill=LUZ)) +
  geom_bar( aes(group = LUZ), position = "dodge", stat="identity") +
  geom_errorbar(aes(xmin=SE_lower, xmax=SE_upper),
                size = 0.5, width = 0.5, position = position_dodge(0.9)) +
