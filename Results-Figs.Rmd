...
emm <- emmeans(m, pairwise~LUZ, type="response", level=0.95, infer=T)
# emm <- emmeans(m, "LUZ")
# plot(emm, comparisons = T)
# pwpp(emm, type = "pairwise")
...
```{r interactions-and-main-effects}
...
# Interactions
    ...
    write.csv(drop1_item,"drop1_interaction.csv", row.names = FALSE)
    
    #dataframe of items where covid and luz interaction is significant
    d_int <- drop1_item %>%
      filter(p.value < 0.05)
    
    d_int_vec <- drop1_item %>%
      filter(p.value < 0.05) %>%
      distinct(item)%>%
      pull(item) %>% 
      droplevels()
...
# Main effects
    # Purrr to look at main effect of covid and luz
    drop1_main = purrr::map_df(d_main_vec, function(x){
...
d_covid_vec <- d_main %>%
  filter(p.value<0.05 & Df=="1") %>%
  distinct(item)%>%
  pull(item) %>% 
  droplevels()

d_luz_vec <- d_main %>%
  filter(p.value<0.05 & Df=="3") %>%
  distinct(item)%>%
  pull(item) %>% 
  droplevels()

d <- rbind(d_int, d_main)

write.csv(d,"drop1_main.csv", row.names = T)

```
# GLMM summaries for interaction items and figure 6 graph

```{r items-interaction, warning=FALSE, message=FALSE}
### Run chunk 8 first to get vector of interaction items ###
 
# Purrr
pred_int_items = purrr::map_df(d_int_vec, function(x){
...
# New facet label names for covid variable
pred_int_items$covid <- factor(pred_int_items$covid, levels = c("1", "2"), 
                        labels = c("Before Covid", "During Covid"), ordered = T)

pred_int_order <- pred_int_items %>%
  group_by(item) %>%
...
pred_int_items$item <- factor(pred_int_items$item, levels = pred_int_order)
# 
#  #create plot
...
#Reorder within interaction items
# pred_int_items <- pred_int_items %>% 
#   mutate(LUZ = as.factor(LUZ),
#   item=as.factor(item),
#   item=reorder_within(LUZ, Total, item))

p_pred_int_items <- 
  ggplot(pred_int_items, aes(x=LUZ, y=Total, fill=covid)) +
  geom_bar(position = "dodge", stat="identity") +
  geom_errorbar(aes(ymin=SE_lower, ymax=SE_upper, width = 0.9),
...
### Run chunk 8 first to get vector of main effect items ###
 
# Purrr Covid
pred_cov_items = purrr::map_df(d_covid_vec, function(x){
...
# New facet label names for covid variable
pred_cov_items$covid <- factor(pred_cov_items$covid, levels = c("1", "2"), 
                        labels = c("Before Covid", "During Covid"), ordered = T)
...
p_pred_cov_items <- 
  ggplot(pred_cov_items, aes(x=covid, y=Total, fill=covid)) +
  geom_bar(position = "dodge", stat="identity") +
  geom_errorbar(aes(ymin=SE_lower, ymax=SE_upper, width = 0.9),
                position = "dodge", width = 0.9) +
  scale_fill_manual(values = c("Before Covid" = "#18BDC3", "During Covid" = "#F3756E")) +
...
```{r items-luz-effect, warning=FALSE, message=FALSE}
### Run chunk 8 first to get vector of main effect items ###
 
# Purrr Land Use
pred_luz_items = purrr::map_df(d_luz_vec, function(x){
...
#create vector from order items
pred_luz_order <- pred_luz_order$item
pred_luz_order <- unlist(pred_luz_order)
pred_luz_items$item <- factor(pred_luz_items$item, levels = pred_luz_order)

# scale_fill_manual(values=c('#18BDC3','#F3756E'))


p_pred_luz_items <- 
  ggplot(pred_luz_items, aes(x=LUZ, y=Total, fill=LUZ)) +
  geom_bar(position = "dodge", stat="identity") +
  geom_errorbar(aes(ymin=SE_lower, ymax=SE_upper, width = 0.9),
                position = "dodge", width = 0.9) +
  # scale_fill_manual(values = c("Before Covid" = "#18BDC3", "During Covid" = "#F3756E")) +
...
# Function loop: each item top txt effect
    # Purrr to determine interaction between covid and luz
    emm_int_cl_top = purrr::map_df(top_txt, function(x){
    ...
    # Purrr to determine interaction between covid and luz
    emm_int_lc_top = purrr::map_df(top_txt, function(x){
  ...
  write.csv(emm_int_lc_top,"emm_lc_top.csv", row.names = T)
  write.csv(emm_int_cl_top,"emm_cl_top.csv", row.names = T)
 
# Purrr to determine interaction between covid and luz
emm_int_cl = purrr::map_df(d_int_vec, function(x){
...
# Purrr to determine interaction between covid and luz
emm_int_lc = purrr::map_df(d_int_vec, function(x){
...
emm_main_cl = purrr::map_df(d_main_vec, function(x){
...
# Purrr to determine interaction between covid and luz
emm_main_lc = purrr::map_df(d_main_vec, function(x){
...
emm_lc <- rbind(emm_int_lc, emm_main_lc)
emm_cl <- rbind(emm_int_cl, emm_main_cl)
...
# Function loop: each item top txt effect
    # Purrr to determine interaction between covid and luz
    emm_main_luz = purrr::map_df(d_main_vec, function(x){
  ...
  write.csv(emm_main_luz,"emm_main_luz.csv", row.names = T)

```

```{r main-effect-luz-graph}
# Purrr
pred_main_df = purrr::map_df(top_txt, function(x){
...
covid_effect_vec <- drop1_main %>%
  filter(item %in% c("Paper  packaging", "OH & S", "Glass / ceramic scrap", "Hard plastic lids", "Hard plastic remnants", "Aluminium cans", "Foil (including bladders)")) %>%
  distinct(item)%>%
  pull(item) %>% 
  droplevels()

luz_effect_vec <- drop1_main %>%
  filter(item %in% c("Hard plastic remnants", "Foil (including bladders)", "Sanitary items", "Plastic bags")) %>%
  distinct(item)%>%
  pull(item) %>% 
  droplevels()

# New facet label names for covid variable
pred_main_df$covid <- factor(pred_main_df$covid, levels = c("2", "1"), 
                        labels = c("During Covid", "Before Covid"), ordered = T)

covid_effect_df <- pred_main_df %>%
  filter(item %in% covid_effect_vec) %>%
  group_by(item, covid) %>%
  mutate(Total = mean(Total),
      covid = as.factor(covid),
      item=as.factor(item),
      item=reorder(item, Total))

p_pred_covid_df <- 
  ggplot(covid_effect_df, aes(x=item, y=Total, fill=covid)) +
  geom_bar(position = "dodge", stat="identity") +
  geom_errorbar(aes(ymin=SE_lower, ymax=SE_upper, width = 0.9),
                position = "dodge", width = 0.9) +
  scale_fill_manual(values = c("Before Covid" = "#18BDC3", "During Covid" = "#F3756E"))
