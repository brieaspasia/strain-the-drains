...
```
```{r percent-change-bootstrap}

pct_change <- df %>%
  filter(item %in% top15 | item == "OH & S") %>% #select items
  group_by(covid, asset_id, item) %>% #look at items per asset both cov
  mutate(assetMean=mean(sum)) %>% #mean of item btwn 8 cycles at each asset per covid level
  ungroup() %>%
  dplyr::select(covid, asset_id, item, assetMean) %>% #select vars
  unique() #COMMENTED HERE %>% #remove duplicates
  # spread(asset_id, assetMean) %>% #assets to cols
  # mutate(itemMean = 
  #          rowMeans(.[,3:122])) %>% #mean items btwn asset means per covid
  # pivot_longer(
  #   cols = DC01:WC20,
  # names_to = "asset_id", 
  # values_to = "assetMean") # %>%
...
# Custom function
mean.fnc = function(x,i){mean(x[i])}

# Loop per item
item_bootstrap = purrr::map_df(ls.data_split, function(x){
...
x_mean = left_join(x1, x2, by = "asset_id") %>% 
  mutate(pc = ((assetMean.y - assetMean.x)/assetMean.x)*100)

# Calculate standard error per item
item.boot = boot(x_mean$pc, mean.fnc, 1000, stype = "i")

# Output array
item = unique(x$item) %>% droplevels()
avg = sum(x_mean$pc)/240
ci = sd(item.boot$t)

out = data.frame(item, avg, ci)
...
item_delta = purrr::map_df(ls.data_split, function(x){
