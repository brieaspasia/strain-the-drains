...
```{r avg-items-per-cycle}

# c_items <- df %>%
#   group_by(cycle, LUZ, item) %>%
#   summarise(total = sum(sum)) %>%
#   arrange(desc(total)) %>%
#   top_n(25) 
# 
# lids <- c_items %>%
#   filter(item=="Plastic drink bottles")
# 
# ggplot(lids, aes(cycle, total)) +
#   geom_col() +
#   facet_wrap(~LUZ)
#   # facet_wrap(~item, scales="free_y") 
#   # + scale_x_discrete(labels=c("1" = "No lockdown", "2" = "Moderate lockdown",
...
# 
# btl <- df %>%
#   filter(cycle=="C7", item=="Plastic drink bottles", LUZ=="Public Transport Terminal")
```
