...
ggplot(item_per_cycle, aes(date, cycle_sum, colour=LUZ)) +
  geom_rect(aes(xmin=xstart, xmax=xend, ymin= -Inf, ymax=Inf, fill=covid)) +
  geom_line() +
  facet_wrap(~item) +
  scale_y_log10()
...
ggplot(item_per_cycle, aes(date, cycle_sum, group=LUZ, colour=LUZ)) +
  geom_point() +
  geom_line() +
  facet_wrap(~item, scale="free_y")
...
ggplot() +
  # geom_polygon(data=hull.data,aes(x=NMDS1,y=NMDS2,fill=covid,group=covid),alpha=0.1) +
  geom_text(data=species.scores,aes(x=NMDS1,y=NMDS2, label=item)) +
