...
p_timeseries <- ggplot() +
  ...
  p_timeseries <- ggplot() +
  geom_line(data=item_per_cycle, aes(cycle, cycle_sum, color=LUZ, group=1)) +
  # theme(axis.text.x=element_text(angle=45, vjust=0.75, hjust=0.8)) +
  facet_wrap(~item)
```
```{r coffee-plot-item}
coffee_sum = df %>% group_by(LUZ, item) %>% summarise(total = sum(sum), sd = sd(sum))


p <- ggplot(coffee_sum, aes(x = LUZ, y = total, fill = item)) +
        ...
        labs(subtitle = "Count per item type and land use from Oct 2019 - May 2020", y = "Coffee-related item count", x="") +
...
```{r coffee-plot-LGA}
coffee_sum = df %>% group_by(LUZ, LGA) %>% summarise(total = sum(sum), sd = sd(sum))

# df_item = df_item %>% mutate(sum = sum*10^-3, sd = sd*10^-3) %>% filter(sum >0.2)

p <- ggplot(coffee_sum, aes(x = LUZ, y = total, fill = LGA)) +
        geom_bar(stat="identity", position="dodge") +
        geom_text(aes(y = total + sd, label = round(total,2)), 
                  position = position_dodge(width =0.9), 
                  vjust = -0.5, color = "black", size = 3) +
        ...
        labs(subtitle = "Count per land use and LGA from Oct 2019 - May 2020", y = "Item count", x="") +
        theme(axis.text.x=element_text(color = "black", size=11, 
                                 vjust=.8, angle=30, hjust=0.8))
