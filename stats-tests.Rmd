...
```{r create-micro-df, warning=FALSE, message=FALSE, results='hide', include=FALSE}
#create survey count  
...
rm(mic_df, micro, micro_abund, micro_abund_long, micro_abund_wide, micro_C8, micro_C9, micro_event, micro_item, micro_site, micro_survey_count, micro_t)    
```
...
micro_d1 <- drop1(micro_m, test="Chisq")

micro_d1

micro_so <- simulateResiduals(fittedModel = micro_m, plot=T)
micro_so

micro_gsum <- summary(micro_m)
micro_gsum <- as.data.frame(micro_gsum$coefficients$cond)
write.csv(micro_gsum,"micro_gsum.csv", row.names = T)

micro_ecl <- emmeans(micro_m, pairwise~covid|LUZ, type="response",level=0.95, infer=T)
micro_ecl <- summary(micro_ecl$contrasts)
write.csv(micro_ecl,"micro_ecl.csv", row.names = T)

micro_elc <- emmeans(micro_m, pairwise~LUZ|covid, type="response",level=0.95, infer=T)
micro_elc <- summary(micro_elc$contrasts)
write.csv(micro_elc,"micro_elc.csv", row.names = T)
